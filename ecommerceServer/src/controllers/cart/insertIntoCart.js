    ...
    router
        .post('/', isAuthenticated, async (req, res) => {
            ...
            try {
                ...
                const cartItem = await findItem(products, ObjectId(item.product_id), item.size, ObjectId(item._id));

                if (cartItem) {

                    // Insert the new cart item into the "cart" collection
                    const newCartItem = await cart.insertOne({
                        user_id: ObjectId(user),
                        product_id: ObjectId(cartItem.product_id),
                        size: cartItem.size
                    });
